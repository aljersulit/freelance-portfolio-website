...
interface EtherealEmailTestTransportOptions extends nodemailer.TransportOptions {
  host: string;
  port: number;
  auth: {
    user: string;
...
export async function sendEmail(mailOptions: nodemailer.SendMailOptions) {
  try {
    ...
    let mailConfig: OAuth2TransportOptions | EtherealEmailTestTransportOptions;
